...
public class MembraneServlet extends HttpServlet {
  ...
  public void init(ServletConfig config) throws ServletException {
    try {
      ...
      log.debug("loading beans configuration from: " + getProxiesXmlLocation(
              config));
      router = RouterUtil.initializeRoutersFromSpringWebContext(appCtx, config.
              getServletContext(), getProxiesXmlLocation(config));
      if (router == null) {
        throw new RuntimeException("No <router> with a <servletTransport> was "
                + "found. To use <router> with <transport>, use "
                + "MembraneServletContextListener instead of MembraneServlet.");
      }
