...
public class MembraneServletContextListener implements ServletContextListener {
  ...
  private static Log log = LogFactory.getLog(
          MembraneServletContextListener.class);
  ...
  public void contextInitialized(ServletContextEvent sce) {
    try {
      ...
      log.debug("loading proxies configuration from: " + getProxiesXmlLocation(
              sce));

      appCtx = new BaseLocationXmlWebApplicationContext();
      Router router = RouterUtil.initializeRoutersFromSpringWebContext(appCtx,
              sce.getServletContext(), getProxiesXmlLocation(sce));
      if (router != null) {
        throw new RuntimeException( "A <router> with a <servletTransport> "
                + "cannot be used with MembraneServletContextListener. "
                + "Use MembraneServlet instead.");
      }
